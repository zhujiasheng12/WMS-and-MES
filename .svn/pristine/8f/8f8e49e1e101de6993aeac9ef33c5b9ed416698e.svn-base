...
namespace WebApplication2.Model.生产管理.精度校验
{
    /// <summary>
   ...
   //         "id":11,
            ...
            List<生产管理.维保.NodeJson> jsons = new List<生产管理.维保.NodeJson>();
        ...
        public static void AddNodes(List<生产管理.维保.NodeJson> jsons, List<JDJS_WMS_Location_Info> nodes, List<JDJS_WMS_Device_Info> deviceNodes, List<JDJS_WMS_Device_Alarm_Repair> ToolInfo)
        {
            ...
            try
            {
                if (jsons.Count == 0)
                {
                    ...
                    {
                        jsons.Add(new 生产管理.维保.NodeJson() { id = item.id, name = item.Name, team = item.EndDate, hierarchy = item.size, });
                    }
                    ...
                    foreach (var json in jsons)
                    {
                        ...
                        if (subNodes.Count() > 0 || DevicesubNodes.Count() > 0)
                        {
                            foreach (var item in subNodes)
                            {
                                ...
                                {
                                    json.children.Add(new 生产管理.维保.NodeJson() { id = item.id, name = item.Name, team = item.EndDate, hierarchy = item.size, flag = -1, CncID = num, state = "closed" });
                                    num--;
                                }
                                else
                                {
                                    json.children.Add(new 生产管理.维保.NodeJson() { id = item.id, name = item.Name, team = item.EndDate, hierarchy = item.size, flag = -1, CncID = num });
                                    num--;
                                }
                            }
                            foreach (var item in DevicesubNodes)
                            {
                                ...
                                json.children.Add(new 生产管理.维保.NodeJson() { id = flag, CncID = item.ID, name = item.MachNum, team = item.IP, hierarchy = item.MachState, flag = error,  });
                                flag--;
                            }
