        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            using (JDJS_WMS_DB_USEREntities wms = new JDJS_WMS_DB_USEREntities())
            {
                ...
                foreach (var order in orders)
                {
                    ...
                    if (guide != null)
                    {
                        info.Time = guide.ExpectEndTime==null?"-": guide.ExpectEndTime.ToString();
                        info.time = guide.ExpectEndTime == null ? DateTime.Now.AddYears(-1) : Convert.ToDateTime ( guide.ExpectEndTime);
                    }
                    
                    var processes = wms.JDJS_WMS_Order_Process_Info_Table.Where(r => r.OrderID == order.Order_ID).FirstOrDefault ();
                    if (processes != null)
                    {
