...
namespace WebApplication2.Model.刀具寿命管理
{
   ...
   //         "id":11,
            ...
            {
                AddNodes(jsons, JDJS_WMS_Device_Info.JDJS_WMS_Location_Info.ToList(), JDJS_WMS_Device_Info.JDJS_WMS_Device_Info.ToList(),JDJS_WMS_Device_Info.JDJS_WMS_Tool_Shank_Table.ToList(), JDJS_WMS_Device_Info.JDJS_WMS_ToolHolder_ToolLife_History_Table.ToList()); ;
        ...
        public void AddNodes(List<NodeJson> jsons, List<JDJS_WMS_Location_Info> nodes ,List<JDJS_WMS_Device_Info> deviceNodes,List <JDJS_WMS_Tool_Shank_Table> toolShank,List<JDJS_WMS_ToolHolder_ToolLife_History_Table> toolLife)
        {
            try
            {
                if (jsons.Count == 0)
                {
                    ...
                    AddNodes(jsons, nodes, deviceNodes, toolShank, toolLife);
                }
                else
                {
                    foreach (var json in jsons)
                    {
                        ...
                        if (subNodes.Count() > 0 || DevicesubNodes.Count() > 0)
                        {
                            ...
                            {
                                var toollist = toolShank.Where(r => r.CncID == item.ID);
                                int error = 0;
                                foreach (var real in toollist)
                                {
                                    var toolId = real.ToolID;
                                    if (toolLife.Where(r => r.ToolID == toolId).Count() > 0)
                                    {
                                        var nowLife =Convert.ToDouble(toolLife.Where(r => r.ToolID == toolId).OrderByDescending(r=>r.Time).First().ToolCurrLife);
                                        var maxLife = Convert.ToDouble(toolLife.Where(r => r.ToolID == toolId).OrderByDescending(r => r.Time).First().ToolMaxLife);
                                        if (nowLife > maxLife)
                                        {
                                            error = 2;
                                            break;
                                        }
                                        else if (nowLife > maxLife*0.8)
                                        {
                                            error = 1;
                                        }
                            ...
                            AddNodes(json.children, nodes, deviceNodes, toolShank, toolLife);
                        }
                        else
                        {
