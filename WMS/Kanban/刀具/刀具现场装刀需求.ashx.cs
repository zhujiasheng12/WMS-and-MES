...
namespace WebApplication2.Kanban.刀具
{
    ...
    /// 刀具现场装刀需求 的摘要说明
    /// </summary>
    public class 刀具现场装刀需求 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            using (JDJS_WMS_DB_USEREntities wms = new JDJS_WMS_DB_USEREntities())
            {
                ...
                {
                    ToolNos.Clear();
                    var standTool = wms.JDJS_WMS_Tool_Standard_Table.Where (r=>r.MachTypeID ==item.MachType);
                    foreach (var moin in standTool)
                    {
                        ToolNos.Add(moin.ToolID);
                    }
            ...
            foreach (var item in toolInfos1)
            {
                ...
                if (!ToolNos .Contains (tool.ToolNo))
                {
                    tool.Type = "特殊刀具";
                }
                else
