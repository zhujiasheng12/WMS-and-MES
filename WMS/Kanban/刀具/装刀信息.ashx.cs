...
namespace WebApplication2.Kanban.刀具
{
    ...
    /// 装刀信息 的摘要说明
    /// </summary>
    public class 装刀信息 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
                            ...
                            if (processInfo.Count() > 0)
                            {
...

                                {
                                    ...
                                    for (int i = 0; i < maxNum; i++)
                                    {
                                        ...
                                        {
                                            string position="";
                                            var rows = CIE.JDJS_WMS_Location_Info.Where(r => r.id == cnc.Position);
                                            if (rows.Count() > 0)
                                            {
                                                position = rows.First().Name;
                                            }
                                            toolInstallation.position = position;
            ...
            var model = new { data = toolInstallationReads.Take(5), count = toolInstallationReads.Count };
