                ...
                foreach (var order in orders)
                {
                    ...
                    for (int i = 0; i < processes.Count(); i++)
                    {
                        if (i == 0)
                        {
                            ...
                            int outNum = 0;
                            int id = processes.ToList()[i].ID;
                            var processBlank = wms.JDJS_WMS_Warehouse_InOut_History_Table.Where(r => r.ProcessId ==id).FirstOrDefault();
                        ...
                        else
                        {
                            ...
                            int outNum = 0;
                            var id = processes.ToList()[i].ID;
                            var blank = wms.JDJS_WMS_Warehouse_InOut_History_Table.Where(r => r.ProcessId == id).FirstOrDefault();
                            if (blank != null)
                            {
                            ...
                            if (blankLast != null)
                            {
                                outNum  = Convert.ToInt32(blankLast.OutNum);
                            }
                            if (outNum < inNum)
                            {
                                VATStatus status = new VATStatus();
