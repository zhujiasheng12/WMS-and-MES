...
namespace WebApplication2.Model
{
    ...
    /// </summary>
    public class machSearch : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            using (JDJS_WMS_DB_USEREntities pm2 = new JDJS_WMS_DB_USEREntities())
            {
                ...
                var search = user.Where(r => r.MachNum == key|r.Name==key);
                var uses = search.OrderBy(r => r.ID).Skip((page1 - 1) * limit).Take(limit);
                var model = new { code = 0, msg = "", count = search.Count(), data = uses };
