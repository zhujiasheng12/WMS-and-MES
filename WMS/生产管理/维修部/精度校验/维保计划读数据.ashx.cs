    ...
    public class 维保计划读数据 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {

            {
                ...
                {
                    var mainOn = wms.JDJS_WMS_Device_Accuracy_Verification_Plan.Where(r => r.isFlag == 1);
                    foreach (var item in mainOn)
                    {
                        ...
                        maintenanceRead.MaintenanceContence = item.Name.ToString();
                        maintenanceRead.MaintenanceCycle = item.Cycle.ToString();
                        maintenanceRead.MaintenanceEnable = Convert.ToInt32(item.isFlag);
                        maintenanceRead.MaintenanceTime = item.AccuracyVerificationTime.ToString();
                        maintenanceRead.MaintenanceGuide = "";
                        maintenanceRead.MaintenacneCycleStr = item.TimeStr;
                        maintenanceRead.NextMaintenTime = Convert.ToDateTime(item.AccuracyVerificationTime).AddDays(Convert.ToDouble(item.Cycle)).ToString();
                        maintenanceRead.days = (Convert.ToDateTime(item.AccuracyVerificationTime).AddDays(Convert.ToDouble(item.Cycle)) - DateTime.Now).Days.ToString() + "天";
                    ...
                    var mainOff = wms.JDJS_WMS_Device_Accuracy_Verification_Plan.Where(r => r.isFlag == 0);
                ...
                var model = new { code = 0, data = maintenanceReads, count = maintenanceReads.Count };
