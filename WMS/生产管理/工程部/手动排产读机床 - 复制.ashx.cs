...
namespace WebApplication2.生产管理.工程部.t
{
    /// <summary>
    /// 手动排产读机床 的摘要说明
    /// </summary>
    public class 手动排产读机床 : IHttpHandler
    {
            ...
            public void ProcessRequest(HttpContext context)
            {
                ...
                var cncType = context.Request["cncType"];
                    AddNodes(jsons, JDJS_WMS_Device_Info.JDJS_WMS_Location_Info.ToList(), JDJS_WMS_Device_Info.JDJS_WMS_Device_Info.ToList(), JDJS_WMS_Device_Info.JDJS_WMS_Tool_LifeTime_Management.ToList(),cncType); 
        ...
        public void AddNodes(List<NodeJson> jsons, List<JDJS_WMS_Location_Info> nodes, List<JDJS_WMS_Device_Info> deviceNodes, List<JDJS_WMS_Tool_LifeTime_Management> ToolInfo, string DeviceType)
        {
            try
            {
                if (jsons.Count == 0)
                {
                    ...
                    AddNodes(jsons, nodes, deviceNodes, ToolInfo, DeviceType);
                }
                else
                {
