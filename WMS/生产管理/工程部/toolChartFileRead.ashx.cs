...
namespace WebApplication2.生产管理.工程部
{
    ...
    /// fileRead 的摘要说明
    /// </summary>
    public class toolChartFileRead : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var processId = int.Parse(context.Request.QueryString["processId"]);
            var fileName = "";
            using (JDJS_WMS_DB_USEREntities entities = new JDJS_WMS_DB_USEREntities())
            {
                fileName = entities.JDJS_WMS_Order_Process_Info_Table.Where(r => r.ID == processId).First().toolChartName;
            }
            PathInfo pathInfo = new PathInfo();
            string path = Path.Combine(pathInfo.upLoadPath(), number,"刀具表");
