...
namespace WebApplication2.生产管理.工程部.处理生产关联订单
{
    ...
    /// 需要修改工艺 的摘要说明
    /// </summary>
    public class 需要修改工艺 : IHttpHandler
    {
                    ...
                    try
                    {
                        ...
                        if (blankInfo != null)
                        {
                            JDJS_WMS_Order_Blank_Table jdBlank = new JDJS_WMS_Order_Blank_Table()
                            {
                                ...
                                BlankAddition = 0,
                                BlankSpecification = blankInfo.BlankSpecification.Contains("#1#") ? blankInfo.BlankSpecification : blankInfo.BlankSpecification+"#1#",
                                BlankState = "待备料",
                        ...
                        foreach (var item in processes)
                        {
                            JDJS_WMS_Order_Process_Info_Table jd = new JDJS_WMS_Order_Process_Info_Table()
                            {
                                ...
                                JigSpecification = item.JigSpecification.Contains("#1#") ? item.JigSpecification : item.JigSpecification+ "#1#",
                                ...
                                BlankSpecification = item.BlankSpecification.Contains("#1#") ? item.BlankSpecification : item.BlankSpecification+ "#1#",
